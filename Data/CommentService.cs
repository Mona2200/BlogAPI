...
namespace BlogProject.Data
{
   public class CommentService
   {
      ...
      public async Task<CommentViewModel> GetCommentViewModel(Comment comment)
      {
         ...
         commentsViewModel.Post = await _db.Post.Where(p => p.Id == comment.PostId).FirstOrDefaultAsync();
         commentsViewModel.User = await _db.User.Where(u => u.Id == comment.UserId).FirstOrDefaultAsync();

         commentsViewModel.Id = comment.Id;
         commentsViewModel.Content = comment.Content;
      ...
      public async Task<CommentViewModel> GetCommentViewModelById(Guid id)
      {
         ...
         var commentsViewModel = await GetCommentViewModel(comment);
         return commentsViewModel;
      }
